...
export const requestMetricsMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on("finish", () => {
        ...
        const normalizedRoute = normalizeRoute(req.originalUrl || req.url || "unknown");
        const statusFamily = `${Math.floor(res.statusCode / 100)}xx`;

        void incrementMetric(MetricKeys.httpRequestsTotal, 1);
        void incrementMetric(`http_requests_total|${req.method}|${normalizedRoute}|${statusFamily}`, 1);
