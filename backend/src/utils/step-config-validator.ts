...
const validateFilterConfig = (stepConfig: unknown) => {
    if (!isPlainObject(stepConfig)) {
        throw new AppError("filter stepConfig must be an object", "INVALID_STEP_CONFIG", 400);
    }
    ...
    if (!isNonEmptyString(field)) {
        throw new AppError("filter.field must be a non-empty string", "INVALID_STEP_CONFIG", 400);
    }

    if (typeof operator !== "string" || !FILTER_OPERATORS.includes(operator as (typeof FILTER_OPERATORS)[number])) {
        throw new AppError("filter.operator is invalid", "INVALID_STEP_CONFIG", 400, {
...
const validateTransformRename = (rename: unknown) => {
    ...
    if (!isPlainObject(rename)) {
        throw new AppError("transform.rename must be an object", "INVALID_STEP_CONFIG", 400);
    }

    const entries = Object.entries(rename);
    if (entries.length === 0) {
        throw new AppError("transform.rename must include at least one key", "INVALID_STEP_CONFIG", 400);
    }

    for (const [fromKey, toKey] of entries) {
        if (!isNonEmptyString(fromKey) || !isNonEmptyString(toKey)) {
            throw new AppError("transform.rename keys and values must be non-empty strings", "INVALID_STEP_CONFIG", 400);
        }
    }
...
const validateTransformAdd = (add: unknown) => {
    ...
    if (!isPlainObject(add)) {
        throw new AppError("transform.add must be an object", "INVALID_STEP_CONFIG", 400);
    }

    if (Object.keys(add).length === 0) {
        throw new AppError("transform.add must include at least one key", "INVALID_STEP_CONFIG", 400);
    }
...
const validateTransformRemove = (remove: unknown) => {
    ...
    if (!Array.isArray(remove)) {
        throw new AppError("transform.remove must be an array", "INVALID_STEP_CONFIG", 400);
    }

    if (remove.length === 0) {
        throw new AppError("transform.remove must include at least one field", "INVALID_STEP_CONFIG", 400);
    }

    if (!remove.every((item) => isNonEmptyString(item))) {
        throw new AppError("transform.remove items must be non-empty strings", "INVALID_STEP_CONFIG", 400);
    }
...
const validateTransformConfig = (stepConfig: unknown) => {
    if (!isPlainObject(stepConfig)) {
        throw new AppError("transform stepConfig must be an object", "INVALID_STEP_CONFIG", 400);
    }
...
const validateEnrichHttpConfig = (stepConfig: unknown) => {
    if (!isPlainObject(stepConfig)) {
        throw new AppError("enrich_http stepConfig must be an object", "INVALID_STEP_CONFIG", 400);
    }

    ensureNoUnknownKeys(stepConfig, ["url", "method", "timeoutMs"], "enrich_http");
    ...
    if (!isNonEmptyString(url)) {
        throw new AppError("enrich_http.url must be a non-empty string", "INVALID_STEP_CONFIG", 400);
    }

    if (!/^https?:\/\//i.test(url.trim())) {
        throw new AppError("enrich_http.url must start with http:// or https://", "INVALID_STEP_CONFIG", 400);
    }

    if (method !== undefined && method !== "GET") {
        throw new AppError("enrich_http.method currently supports only GET", "INVALID_STEP_CONFIG", 400, {
            allowedMethods: ["GET"],
        });
    }
...
export const validateStepConfigByType = (stepType: string, stepConfig: unknown) => {
    switch (stepType) {
        case "filter":
