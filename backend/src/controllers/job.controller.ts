...
export const getJobsHandler = async (req: AuthRequest, res: Response) => {
  ...
  return sendSuccessResponse(res, "Jobs retrieved successfully", { jobs }, 200);
};
...
export const getJobByIdHandler = async (req: AuthRequest, res: Response) => {
  ...
  return sendSuccessResponse(res, "Job retrieved successfully", { job }, 200);
};

export const getPipelineJobsHandler = async (
  req: AuthRequest,
  res: Response,
) => {
  if (!req.user) throw new AppError("Unauthorized", "UNAUTHORIZED", 401);
  const jobs = await getPipelineJobsService(
