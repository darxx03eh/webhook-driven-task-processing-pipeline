...
export const getJobsService = async (userId: string) => findJobsByUserId(userId);
...
export const getPipelineJobsService = async (pipelineId: string, userId: string) => {
    const jobs = await findJobsByPipelineIdForUser(pipelineId, userId);
    if(!jobs)
        throw new AppError("No jobs found for this pipeline", "JOBS_NOT_FOUND", 404);
    return jobs;
}
