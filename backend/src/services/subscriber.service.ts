...
export const createSubscriberService = async (input: CreateSubscriberInput) => {
    const pipeline = await findPipelineOwnedByUserForSubscribers(input.pipelineId, input.userId);
    if(!pipeline)
...
export const getPipelineSubscribersService = async (pipelineId: string, userId: string) => {
    const pipeline = await findPipelineOwnedByUserForSubscribers(pipelineId, userId);
    if(!pipeline)
        throw new AppError("Pipeline not found", "PIPELINE_NOT_FOUND", 404);
    return findSubscribersByPipelineId(pipelineId);
}

export const deleteSubscriberService = async (subscriberId: string, userId: string) => {
