...
import { MetricKeys, durationCountKey, durationSumKey } from "../../../shared/metrics/metric-keys";
import {
...
export const getMetricsSnapshotService = async (userId: string) => {
    ...
    const [jobs, oldestPendingAgeSecondsRaw, jobDuration, delivery] = await Promise.all([
    ...
    return {
        ...
        counters: {
            ...
            webhooks_invalid_signature_total: toNumber(runtimeSnapshot[MetricKeys.webhooksInvalidSignatureTotal]),
            http_requests_total: toNumber(runtimeSnapshot[MetricKeys.httpRequestsTotal]),
            rate_limited_requests_total: toNumber(runtimeSnapshot[MetricKeys.rateLimitedRequestsTotal]),

            jobs_claimed_total: toNumber(jobs.processing_count) + toNumber(jobs.completed_count) + toNumber(jobs.failed_count) + toNumber(jobs.filtered_out_count),
        ...
        durations: {
            webhook_ingest_duration_ms: readDuration(runtimeSnapshot, MetricKeys.webhookIngestDurationMs),
            http_request_duration_ms: readDuration(runtimeSnapshot, MetricKeys.httpRequestDurationMs),
            ...
            delivery_attempt_duration_ms: readDuration(runtimeSnapshot, MetricKeys.deliveryAttemptDurationMs)
        },
        gauges: {
