...
export const executePipelineSteps = async (
    ...
    for(const step of steps) {
        switch(step.stepType) {
            case "filter": {
                const result = applyFilterStep(currentPayload, step.stepConfig as FilterStepConfig);
                if(result.stopped) return result;
                currentPayload = result.payload;
                break;
            }
            case "transform": {
                currentPayload = applyTransformStep(currentPayload, step.stepConfig as TransformStepConfig);
                break;
            }
            case "enrich_http": {
                currentPayload = await applyEnrichHttpStep(currentPayload, step.stepConfig as EnrichHttpStepConfig);
                break;
            }
            default: throw new Error(`Unsupported step type: ${step.stepType}`);
        }
    }
    return {
        stopped: false,
